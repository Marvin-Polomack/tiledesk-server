...
class Listener {
    ...
    listen(config) {
        ...
        if (host === "redis.railway.internal") {
            winston.info("Railway internal Redis detected - trying alternative Redis configuration for WhatsApp connector");
            
            // Try using Railway's public Redis URL if available
            if (process.env.REDIS_URL) {
                winston.info("Using REDIS_URL for WhatsApp connector");
                // Parse Redis URL to extract components
                const redisUrl = new URL(process.env.REDIS_URL);
                host = redisUrl.hostname;
                port = redisUrl.port || 6379;
                var password = redisUrl.password || process.env.CACHE_REDIS_PASSWORD;
                winston.info(`WhatsApp connector using parsed Redis URL: ${host}:${port}`);
            } else {
                winston.info("No REDIS_URL available - disabling WhatsApp connector to avoid IPv6 connection issues");
                return;
            }
        } else {
